...
class Project(Base):
    ...
    # Per-project PM rules (appended after global rules)
    custom_rules = Column(Text, nullable=True)

    # Pause after analysis for pre-decompose PM discussion
    pause_after_analysis = Column(Boolean, default=False)
    pm_context_notes = Column(Text, nullable=True)  # notes collected during pause, injected into decompose prompt
    ...
    # Completion report (generated by PM when project completes)
    completion_report = Column(JSON, nullable=True)
...
class Task(Base):
    ...
    # Task archive (completed tasks can be hidden)
    archived = Column(Boolean, default=False)

    # Rate-limit 자동 재시도 예약
